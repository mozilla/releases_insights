...
class Release
{
    ...
    public function getFutureSchedule(): array
    {
        ...
        $schedule = [
            ...
            'qa_test_plan_due'       => match ($this->version->normalized) {
                '148.0' => $date('Friday +1 weeks 21:00'),
                default => $date('Friday 21:00'),
            },
    ...
    public static function getNiceLabel(string $version, string $label, bool $short=true): string
    {
        ...
        $labels = [
            ...
            'merge_day'             => 'Merge day',
            'beta_1_gtb'            => ($short ? '' : 'Firefox ') . $short_version . ' b1 GTB',
            'beta_1'                => ($short ? '' : 'Firefox ') . $short_version . ' b1',
