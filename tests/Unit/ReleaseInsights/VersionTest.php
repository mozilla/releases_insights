<?php
declare(strict_types=1);

use ReleaseInsights\Version;
use ReleaseInsights\ESR;


test('Version::get', function () {
    $this->assertEquals(BETA . '.0', Version::get());
    $this->assertEquals(NIGHTLY . '.0', Version::get(FIREFOX_NIGHTLY));
    $this->assertEquals(BETA . '.0', Version::get(FIREFOX_BETA));
    $this->assertEquals(RELEASE . '.0', Version::get(FIREFOX_RELEASE));
    $this->assertEquals('100.0', Version::get('100'));
    $this->assertEquals('1200.0', Version::get('1200'));

    // With GET
    $_GET['version'] = 'release';
    $this->assertEquals(RELEASE . '.0', Version::get());
    $_GET['version'] = 'beta';
    $this->assertEquals(BETA . '.0', Version::get());
    $_GET['version'] = 'nightly';
    $this->assertEquals(NIGHTLY . '.0', Version::get());
    $_GET['version'] = 'esr';
    $this->assertEquals((int) ESR . '.0', Version::get());
    $_GET['version'] = '90';
    $this->assertEquals('90.0', Version::get());

});

test('Version::decrement', function ($version, $number, $output) {
    expect($output)->toEqual(Version::decrement($version, $number));
})->with([
    ['1.0', 1, '1.0'],
    ['1.6', 2, '1.0'],
    ['91', 2, '89.0'],
    ['91.0', 2, '89.0'],
    ['100', 3, '97.0'],
]);

test('Version::getMajor', function ($input, $output) {
    expect($output)->toEqual(Version::getMajor($input));
})->with([
    ['91.1.0', 91],
    ['100', 100],
    ['100.5', 100],
    ['78.0.3', 78],
    ['', null],
    [null, null],
]);